...
class SupplierFirebaseScreen extends StatefulWidget {
  const SupplierFirebaseScreen({super.key});

  @override
  State<SupplierFirebaseScreen> createState() => _SupplierFirebaseScreenState();
}

class _SupplierFirebaseScreenState extends State<SupplierFirebaseScreen> {
  ...
  Stream<QuerySnapshot> getCustomers() {
    return FirebaseFirestore.instance.collection('supplier').orderBy('name').snapshots();
  }
      ...
      builder: (context) => AlertDialog(
        ...
        actions: [
          ...
          ElevatedButton(
            ...
            onPressed: () async {
              await FirebaseFirestore.instance.collection('supplier').doc(docId).delete();
              Navigator.pop(context);
              ScaffoldMessenger.of(context).showSnackBar(
      ...
      child: Row(
        children: [
          ...
          Column(
            ...
            children: [
              Text("Total Supplier", style: TextStyle(color: Colors.white70, fontSize: 13)),
              const SizedBox(height: 4),
              Text("$totalCount Supplier", style: const TextStyle(color: Colors.white, fontSize: 20, fontWeight: FontWeight.bold)),
            ],
          ),
        ],
      ),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text("Supplier List", style: TextStyle(fontWeight: FontWeight.bold)),
        centerTitle: true,
      ),
                  ...
                  children: [
                    ...
                    Expanded(
                      child: filtered.isEmpty
                          ? const Center(child: Text("No Supplier found", style: TextStyle(color: Colors.grey)))
                          : ListView.builder(
